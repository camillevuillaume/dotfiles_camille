...
config.load_autoconfig(False)  # don't load settings done via the gui


def read_xresources():
    try:
        with open(
            os.path.join(os.path.expanduser("~"), ".Xresources"), "r", encoding="utf-8"
        ) as f_xresources:
            props = {}
            for line in f_xresources.readlines():
                if line.startswith("*"):
                    prop, _, color = line.partition(":")
                    props[prop.strip()] = color.strip()
            return props
